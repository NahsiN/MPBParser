...
def strip_spaces(fields):
    ''' Strip spaces and newline characters from list of strings '''
...
class MPBBandstructure:
    ...
    def csvparser(self):
        '''
        Parses MPB output file to two csv files
        '''
        ...
        for line in f_mpb:
            ...
            # write out a header to the velocites file
    ...
    def readbanddata(self):
        ...
        freqs_data.pop(0)  # pop out top row of string
        velocities_data.pop(0)
        f_freqs.close()
        f_velocities.close()

        # convert list of lists to appropriate numpy arrays
        k = np.zeros((self.numK,3))
        kmag = np.zeros(self.numK)
        freqs = np.zeros((self.numK,self.numBands))
        vg = np.zeros((self.numK,3,self.numBands))
        vgmag = np.zeros((self.numK,self.numBands))

        for i in range(0,self.numK):
            ...
            for j in range(0,self.numBands):
